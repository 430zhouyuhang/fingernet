...
- 说明：`src/` 已适配 TensorFlow 2.x 的图模式（使用 `tf.compat.v1` + `tensorflow.keras`），网络结构与输入输出未变。
- 推荐版本（CPU）：
```
...
```
- 运行方式（工作目录建议设为 `src/`，以保证相对路径如 `../datasets/` 正确）：
```
# 预测 Demo（部署）：
...
```
- PyCharm 运行参数：在 Run/Debug Configurations 的 Parameters 填入：
  - 部署：`0 deploy`
  - 测试：`0 test`
  - 训练：`0 train`
- 额外依赖变更：
  - 图像读写改用 `imageio`（替代旧的 `scipy.misc.imread/imsave`）
  - 使用 Keras legacy 优化器以兼容旧式 graph 训练：`tf.keras.optimizers.legacy.Adam/SGD`
  - SciPy 的高斯窗口改用 `scipy.signal.windows.gaussian`
- 预训练权重：`models/released_version/Model.model` 若非 TF Checkpoint/h5，将无法直接用 `load_weights` 恢复；可先将 `pretrain=None` 跑通流程，或准备匹配格式的权重文件。
