"""
FingerNet 实用函数集合：
- 文件/日志：mkdir、re_mkdir、init_log、copy_file、get_files_in_folder
- 图像/信号：gaussian2d、gabor_fn/gabor_bank、gausslabel
- 标签 I/O：.mnt 读写（mnt_reader/mnt_writer）
- 评估与后处理：mnt_P_R_F、nms、angle/distance 计算
- 可视化：draw_ori_on_img、draw_minutiae

注意：
- 使用 imageio 进行图像读写，避免旧版 scipy.misc 接口
- 使用 scipy.signal.windows.gaussian 替代已弃用的 signal.gaussian
"""
...
def init_log(output_dir):
    """初始化日志：文件输出到 output_dir/log.log，同时开启控制台 INFO 输出。"""
    re_mkdir(output_dir)
...
def point_rot(points, theta, b_size, a_size):
    """围绕图像中心将坐标点旋转 theta（弧度），并从原尺寸坐标映射到目标尺寸坐标。"""
    cosA = np.cos(theta)
    sinA = np.sin(theta)
...
def mnt_reader(file_name):
    """读取 .mnt 细节点标签，返回 [[w, h, o] 或 [w, h, o, confidence], ...]。前两行是头信息。
    
    支持两种格式：
    - 3列：x y orientation (传统格式)
    - 4列：x y orientation confidence (增强格式)
    """
    f = open(file_name)
    minutiae = []
    for i, line in enumerate(f):
        ...
        if len(parts) >= 3:
            w, h, o = float(parts[0]), float(parts[1]), float(parts[2])
            w, h = int(round(w)), int(round(h))
            if len(parts) >= 4:  # 包含confidence信息
                confidence = float(parts[3])
                minutiae.append([w, h, o, confidence])
            else:  # 传统格式
...
def mnt_writer(mnt, image_name, image_size, file_name):
    """将细节点数组写为 .mnt 文件，包含图名与 (N, H, W) 头信息。
    ...
    for i in range(mnt.shape[0]):
        if mnt.shape[1] >= 4:  # 包含confidence信息
...
def gabor_bank(stride=2,Lambda=8):
    """生成 -90..90 度、步长 stride 的 Gabor 滤波器组，返回形状 [25,25,1,num_angles]。"""
...
def gausslabel(length=180, stride=2):
    """生成环状高斯标签窗口（用于方向分布监督），length 为角度范围，stride 为角度步长。"""
...
def mnt_P_R_F(y_true, y_pred, maxd=15, maxo=np.pi/6):
    """计算细节点匹配的 precision/recall/F1，以及平均位置/方向误差。"""
...
def draw_minutiae(image, minutiae, fname, r=15):
    """在图像上绘制细节点与朝向并保存。r 控制箭头长度。"""
    image = np.squeeze(image)
    fig = plt.figure()
    plt.imshow(image,cmap='gray')
    # plt.hold(True)  # removed for matplotlib>=2
...
def draw_ori_on_img(img, ori, mask, fname, coh=None, stride=16):
    """在图像上按 stride 网格绘制方向场箭头；mask==1 的位置才绘制；coh 可调箭头长度。"""
    ori = np.squeeze(ori)
